...
{
  options.sergv.programs.firefox = {
    enable = lib.mkEnableOption "Enable privacy-enhanced firefox";
  };

  config.home-manager.users."${config.sergv.user.name}" = lib.mkIf config.sergv.programs.firefox.enable {

    imports = [ sergv.inputs.arkenfox.hmModules.default ];
        ...
        profiles =
          ...
          lib.mkMerge
            [
              ...
              (lib.mkIf config.sergv.i2p.enable
                {
                  i2p = default // {
                    bookmarks = {
                      force = true;
