...
{
  options.sergv.system.optimized-linux-kernel = {
    enable = lib.mkEnableOption "Build custom Linux kernel tuned for AMD desktop";
  };

  config = lib.mkIf config.sergv.system.optimized-linux-kernel.enable
    {
      boot.kernelPackages =
        let
