...
$ nix-store --realise /nix/store/6ygk7vjfdyqnjkn12r44w04xqjlhxx70-hello-minimalistic.drv
warning: you did not specify '--add-root'; the result might be removed by the garbage collector
/nix/store/x75rn4ym1pl3pin5rg9gf5s357ayzmqa-hello-minimalistic
$ nix-store --query --references /nix/store/x75rn4ym1pl3pin5rg9gf5s357ayzmqa-hello-minimalistic
/nix/store/hp2fa1cjwkyyfhrd6jdr7i2bqbgp4alh-glibc-2.34-210
/nix/store/x75rn4ym1pl3pin5rg9gf5s357ayzmqa-hello-minimalistic
#+END_EXAMPLE

** Show everything used to produce OUTPUT
~nix-store --query --tree $(nix-store --query --deriver OUTPUT)~
...
- paths
  ...
  + ~pkgs.lib.path.append : Path -> String -> Path~
...
** Force add file to store bypassing hash checking
~sudo mount -o remount,rw /nix/store~
~sudo cp file </nix/store/PATH>~
~printf '</nix/store/PATH>\n\n0\n' | sudo nix-store --register-validity --reregister~
** Delete too old or too new system generations
Find generations ~sudo nix-env --list-generations --profile /nix/var/nix/profiles/system~
Set current generation ~sudo nix-env --profile /nix/var/nix/profiles/system --switch-generation XXX~
Remove selected generations ~sudo nix-env --delete-generations --profile /nix/var/nix/profiles/system NNN KKK~
...
* Wine
For faf but helps with others too
- Install dxvk and other dependencies: ~winetricks dxvk d3dx9 xact~
- Update faf-linux repo
- ~export DXVK_CONFIG_FILE="/home/sergey/games/installed/scfa/faf-linux/dxvk.conf"~
  ^may need to use dxvk config from there, not confirmed but seems to work with it
- ~export WINEDLLOVERRIDES="d3d9=n"~  <- from proton launcher
- ~export WINE_LARGE_ADDRESS_AWARE=1~ <- ditto
- ~unset WINEESYNC WINEFSYNC~         <- ditto

– Standard stuff so that redshift won’t mess up gamma:
  ~wine reg add 'HKEY_CURRENT_USER\Software\Wine\X11 Driver' /t REG_SZ /v UseXVidMode /d N /f~

- Run winecfg, enable virtual desktop 3840 by 2160 with default 96 dpi

* Smartctl
- General: ~sudo smartctl -x -a /dev/sdb~
- Run test: ~sudo smartctl --test=short /dev/sdb~
- Show test results: ~sudo smartctl --attributes --log=selftest /dev/sdb~
