...
let
  ...
  makeHaskellPackageSmaller =
    x: hlib.dontHaddock (hlib.disableLibraryProfiling (hlib.disableExecutableProfiling x));
...
in
{
  ...
  smaller-hpkgs-no-ghc =
    hpkgs: hpkgs.extend (_: old: builtins.mapAttrs makeHaskellPackageAttribSmaller old);
