{
  ...
  outputs =
    ...
    let
      ...
      enable-ghc-unit-ids-overlay = new: old: {
        haskell = old.haskell // {
          compiler = builtins.mapAttrs (_: hutils.enable-unit-ids-for-newer-ghc) old.haskell.compiler // {
            native-bignum = builtins.mapAttrs (
              _: hutils.enable-unit-ids-for-newer-ghc
            ) old.haskell.compiler.native-bignum;
          };
        };
      };
      ...
      smaller-haskell-overlay = new: old: {
        haskellPackages =
          hutils.fixedExtend (hutils.smaller-hpkgs old.haskell.packages.native-bignum.ghc966)
            (
              _: old2: {
        ...
        haskell = old.haskell // {
          packages = builtins.mapAttrs (_: hutils.smaller-hpkgs-no-ghc) old.haskell.packages // {
            native-bignum = builtins.mapAtrs (
              _: hutils.smaller-hpkgs-no-ghc
            ) old.haskell.packages.native-bignum;
          };
        };
      };
      ...
      haskell-disable-checks-overlay = _: old: {
        ...
        haskell = old.haskell // {
          packages = old.haskell.packages // {
            ...
            ghc965 = hutils.fixedExtend old.haskell.packages.ghc965 (
              _: old2:
              temporarily-disable-problematic-haskell-pkgs-checks old old2
              // disable-problematic-haskell-crypto-pkgs-checks old old2
            );
            ghc966 = hutils.fixedExtend old.haskell.packages.ghc966 (
              _: old2:
              temporarily-disable-problematic-haskell-pkgs-checks old old2
              // disable-problematic-haskell-crypto-pkgs-checks old old2
            );
      ...
      zen4-march-overlay = _: old: {
        ...
        cabal2nix-unwrapped = old.haskell.lib.justStaticExecutables (
          old.haskell.packages.native-bignum.ghc966.generateOptparseApplicativeCompletions [
            "cabal2nix"
