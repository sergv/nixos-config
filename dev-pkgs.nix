...
let
  # pkgs-pristine = nixpkgs-unstable.legacyPackages."${system}";
  t             = pkgs.lib.trivial;
  ...
  allowGhcReference =
    x:
    hlib.overrideCabal x (drv: {
      disallowGhcReference = false;
    });

  hpkgsDoctest = hpkgs910.extend (
    _: old:
    builtins.mapAttrs hutils.makeHaskellPackageAttribSmaller (
      old
      // {
        doctest = hlib.dontCheck (
  ...
  hpkgsGhcEvensAnalyze = hpkgs96.extend (
    _: old:
    builtins.mapAttrs hutils.makeHaskellPackageAttribSmaller (
      old
      // {
        ghc-events-analyze = hlib.doJailbreak (
  ...
  hpkgsEventlog2html = hpkgs96.extend (
    _: old:
    builtins.mapAttrs hutils.makeHaskellPackageAttribSmaller (
      old
      // {
  ...
  hpkgsProfiterole = hpkgs96.extend (
    _: old:
    builtins.mapAttrs hutils.makeHaskellPackageAttribSmaller (
      old
      // {
  ...
  hpkgsCabal = hpkgs910.extend (
    ...
    builtins.mapAttrs hutils.makeHaskellPackageAttribSmaller (
      ...
      // {
        ...
        #   {};
        Cabal-syntax         = old.callCabal2nix "Cabal-syntax" (cabal-repo + "/Cabal-syntax") { };
        Cabal-tests          = old.callCabal2nix "Cabal-tests" (cabal-repo + "/Cabal-tests") { };
        cabal-install-solver = hlib.doJailbreak (
          old.callCabal2nix "cabal-install-solver" (cabal-repo + "/cabal-install-solver") { }
        );
        # hlib.dontCheck
        ...
        hackage-security = hlib.doJailbreak (old.callHackage "hackage-security" "0.6.2.6" { });
        ...
        witherable = hlib.dontCheck (old.callHackage "witherable" "0.5" { });

        process = hlib.dontCheck (old.callHackage "process" "1.6.25.0" { });

        directory = hlib.dontCheck (old.callHackage "directory" "1.3.9.0" { });
  ...
  threadscopePkgs = pkgs.haskell.packages.ghc928.extend (
    _: old:
    builtins.mapAttrs hutils.makeHaskellPackageAttribSmaller (
      old
      // {
    ...
    pkgs.runCommand ("wrapped-cabal")
      {
        ...
        # taking both.
        buildInputs = pkgs.lib.lists.concatMap (
          x:
          if builtins.hasAttr "dev" x then
            [
              x
              x.dev
            ]
      ...
      ''
        mkdir -p "$out/bin"
        makeWrapper "${pkg}/bin/cabal" "$out/bin/cabal" --suffix "PKG_CONFIG_PATH" ":" "${
          pkgs.lib.makeSearchPathOutput "dev" "lib/pkgconfig" bakedInNativeDeps
        }"
      '';
  ...
  symlink-exe-to =
    ...
    let
      f =
        ...
        let
          suffix = if builtins.isNull dest then "" else "-${dest}";
        in
    ...
    pkgs.runCommand ("wrapped-" + source)
      ...
      ''
        mkdir -p "$out/bin"
        ${if builtins.isList dests then builtins.concatStringsSep "\n" (builtins.map f dests) else f dests}
      '';

  wrap-ghc-arch =
    old-arch-prefix: new-arch-prefix: old-version: new-version: alias-versions: pkg:
    assert (builtins.isString old-version);
    let
      old-prefix = if builtins.isNull old-arch-prefix then "" else "${old-arch-prefix}-";
      ...
      f =
        ...
        in
        ''ln -s "$out/bin/${old-prefix}$x-${old-version}" "$out/bin/${new-prefix}$x${suffix}"'';
    in
  ...
  wrap-ghc-filter-selected-args =
    filtered-args: version: alias-version: pkg:
    let
      wrapped-ghc = pkgs.writeShellScript ("filtering-ghc-" + version) ''
        args=("''${@}")
  ...
  ghc-win =
    let
      ...
      wine = pkgs-cross-win.winePackages.minimal.overrideAttrs (old: {
        patches =
          if (versionGE old.version "9.0") then
            ...
            builtins.filter (
              x: !(pkgs-cross-win.lib.strings.hasSuffix "wine-add-dll-directory.patch") x
            ) old.patches
      ...
      ghc-win =
        (win-pkgs.pkgsBuildHost.haskell-nix.compiler."${latest-ghc-field}".override (_: {
          enableNativeBignum = true;
        })).overrideAttrs
          (old: {
            ...
            buildPhase =
              builtins.replaceStrings [ " --flavour=" ] [ " --hash-unit-ids --flavour=" ]
                old.buildPhase;
      ...
      win-iserv-proxy-interpreter = win-exes.iserv-proxy-interpreter.override (old: {
        ...
        setupBuildFlags = [
          "--ghc-option=-optl-Wl,--disable-dynamicbase,--disable-high-entropy-va,--image-base=0x400000"
        ];
      });
      ...
      # "-L${gmp}/lib"
      wrap-win-ghc =
        pkg: ghc-exe: new-names:
        let
          wrapped = pkgs-cross-win.pkgsBuildBuild.writeShellApplication {
        ...
        in
        symlink-exe-to wrapped wrapped.name new-names;
      ...
      ghc-win-wrapped = wrap-win-ghc ghc-win "x86_64-w64-mingw32-ghc" [
        "ghc-${latest-ghc-short-version}-win"
        ghc-win-exe-name
      ];
      ghc-pkg-win-wrapped = wrap-win-ghc-pkg ghc-win "x86_64-w64-mingw32-ghc-pkg" [
        "ghc-pkg-${latest-ghc-short-version}-win"
        ghc-pkg-win-exe-name
      ];
      hsc2hs-win-wrapped = wrap-win-hsc2hs ghc-win "x86_64-w64-mingw32-hsc2hs" [
      ...
      cabal-win-wrapped =
        let
          test-wrapper =
            ...
            win-pkgs.pkgsBuildBuild.writeScriptBin "cabal-win-test-wrapper" ''
...
// {
  ...
  ghc7103 =
    wrap-ghc-filter-all "7.10.3" "7.10"
      pinned-pkgs.nixpkgs-18-09.haskell.packages.ghc7103.ghc;
  ghc802 =
    wrap-ghc-filter-hide-source-paths "8.0.2" "8.0"
      pinned-pkgs.nixpkgs-18-09.haskell.packages.ghc802.ghc;
  ...
  ghc9122-pie = wrap-ghc-rename latest-ghc-version [ "${latest-ghc-short-version}-pie" "pie" ] (
    relocatable-static-libs-ghc latest-ghc-pkg
  );
  ...
  # lld   = pkgs.lld_19;
  lld = filter-bin "llvmPackages_19.bintools" [
    {
      source  = "ld";
      dest    = "lld";
      aliases = [ "ld.lld" ];
